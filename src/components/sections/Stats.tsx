...
function Stat({ value, label, prefix = '', suffix = '' }: { value: number; label: string; prefix?: string; suffix?: string }) {
  ...
  useGSAP(() => {
    ...
    gsap.to(counter, {
      ...
      onUpdate: () => { targetEl.textContent = `${prefix}${Math.round(counter.val)}${suffix}`; },
  ...
  return (
    <div ref={containerRef} className="stat-item text-center" data-testid={`stat-${label.toLowerCase().replaceAll(' ', '-')}`}>
      <p ref={numberRef} className="stat-number font-display text-[clamp(2.4rem,6vw,4rem)] font-extrabold leading-none tracking-[-.055em] text-[#151a35]">
        {prefix}{value}{suffix}
      </p>
